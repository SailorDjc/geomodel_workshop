...
class GmeModelList(object):
    ...
    def access_model_data(self, **kwargs):
        ...
        if self.data_type == 'Noddy' or len(self.pre_train_model_list) > 0 or len(self.train_model_list) > 0:
    ...
    def process_noddy_data(self):
        ...
        for model_idex in np.arange(len(self.pre_train_model_list)):
            ...
            model_extern = self.get_model_extern(self.pre_train_model_list[model_idex])

            geodata = GeoMeshParse(mesh, name=self.pre_train_model_list[model_idex], normalize=False)
            ...
            self.update_graph_log(model_name=self.pre_train_model_list[model_idex],
                                  save_idx=model_idex + pre_save_graph_num, extern=model_extern,
        ...
        if self.train_model_list is not None:
            ...
            model_extern = self.get_model_extern(self.train_model_list[0])

            geodata = GeoMeshParse(mesh, name=self.train_model_list[0], normalize=False, pre_train=False)
            dgl_graph = geodata.execute(sample_operator=self.sample_operator, extent=model_extern,
                                        edge_feat=self.dgl_graph_param[1], node_feat=self.dgl_graph_param[0],  #
                                        center_random={'x': True, 'y': True},
                                        sample_type={'x': 2, 'y': 3}, drill_num=15)
            ...
            print('is_connected:', is_connected)
            self.geodata.append(geodata)
            self.update_graph_log(model_name=self.train_model_list[0],
    ...
    def process_dat_file(self, **kwargs):
        ...
        if self.data_type == 'Wells':
            geodata.set_wells_from_dat_file(dat_file_path=self.dat_file_path, **kwargs)
        elif self.data_type == 'Points':
            geodata.set_points_from_dat_file(dat_file_path=self.dat_file_path, **kwargs)
            # dat_file_path, file_header=None, index_col=None, names=None, use_cols=None
        ...
        self.update_graph_log(model_name=self.train_model_list[0],
                              save_idx=0, extern=geodata.output_grid_param,
                              node_feat=self.dgl_graph_param[0], edge_feat=self.dgl_graph_param[1],
                              is_pre_train=False)
        if self.predict_graph is None:
            self.predict_graph = []
